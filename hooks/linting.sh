...
solChangedFiles=$(git diff --name-only --diff-filter ACMRT --exit-code --staged -- '***.sol' ':!src/interfaces/*.sol')
isolChangedFiles=$(git diff --name-only --diff-filter ACMRT --exit-code --staged -- 'src/interfaces/*.sol')
...
if [[ ! -z $isolChangedFiles ]]; then
	...
	stderrOutput=$(npx solhint $isolChangedFiles --config src/interfaces/.solhint.json 2>&1) # Capture stderr
	if [[ ! -z $stderrOutput ]]; then
